...
const Project = () => {
  ...
  const { getProject, project, handleModalTaskForm, load, submitTasksProject, deleteTaskProject, updateTaskProject, statusTaskProject } = useProjects()
  ...
  useEffect( () => {
    socket.on('added task', newTask => newTask.project === project._id && submitTasksProject(newTask))
    socket.on('deleted task', deletedTask => deletedTask.project === project._id && deleteTaskProject(deletedTask))
    socket.on('updated task', updatedTask => updatedTask.project._id === project._id && updateTaskProject(updatedTask))
    socket.on('new state', newStateTask => newStateTask.project._id === project._id && statusTaskProject(newStateTask))
  })
